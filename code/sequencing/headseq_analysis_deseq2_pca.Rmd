...
#this is the dataset with normalized counts, will be used in the shiny app
# saveRDS(dds, "data.RDS")
```
...
pca %>%
  ...
  ggplot(., aes(PC1,PC2))+
    ...
    geom_point(aes(shape=tp, fill=condition),size=6,alpha=0.75)+
    # ylim(limits=c(-30,40))+
    # xlim(limits=c(-50,60))+
...
# saveRDS(pca, "pca_data.RDS")
ggsave(file.path(here(),paste0("headseq_PCA_",number_of_variable_genes,"_most_variable_genes",".png")),height=5, width=8)
