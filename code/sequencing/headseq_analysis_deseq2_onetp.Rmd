...
plot_counts_ggplot <- function(geneset,selected_tps){
  ...
  ggplot(counts, aes(y=gene,x=counts_norm, fill=condition))+
    # geom_boxplot(aes(fill=condition),outlier.size = .75)+
    geom_point(position=position_dodge(width = .5),size=2,color="black",shape=21)+
    theme_bw()+
...
# write.table(de_results_filtered$gene_wb,"test.txt",row.names = FALSE,quote = FALSE)
...
ggsave(file.path(here(), paste0("counts_",A_tp1,"_vs_",B_tp1,"_",pattern,".png")))
...
ggplot(data=counts,aes(x=tp,y=counts_norm))+
  geom_pointrange(data=counts, aes(x=tp,y=mean_counts_norm,ymin=mean_counts_norm-sd_mean_counts_norm, ymax=mean_counts_norm+sd_mean_counts_norm,group=condition_tp),alpha=0.5, width=.1,size=1,position=position_dodge(0.2),shape=3)+
  geom_point(aes(fill=condition),position=position_dodge(0.2),size=3,color="black",shape=21)+
