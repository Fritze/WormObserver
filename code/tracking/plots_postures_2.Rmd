...
ggsave(file.path(save_path, "postures_grouped_angle_density_p1vsp2p3.png"),height=3,width=6)
...
ggplot(s)+
  ...
  ggsave(file.path(save_path, "postures_grouped_overlay_OP50_p1vsp2p3.png"),height=3,width=6)
...
s <- cluster_centers_reduced_grouped %>%
  ...
  filter(annotation == "OP50") %>%
  #manually select 2 posture to be highlighted
  mutate(highlight=ifelse(newID2 %in% c(94,23,109),"YES","NO")) %>%
