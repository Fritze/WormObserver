...
#plots
occurence_plot <- function(postures_zscore, selected_annotations){
...
selected_annotations <- c("Agar","OP50","OP50_w_Az","daf2_OP50","OP50_RTdauers")
pal <- wes_palette("Darjeeling1")[c(1,5,2,3,4)]

occurence_plot(postures_zscore, selected_annotations)
ggsave(file.path(save_path,"posture_groups_relative_occurences_norm.png"),height=6,width=10)
...
for (i in unique(data_for_plotting$posture_grouped)){
  ggplot(filter(data_for_plotting, posture_grouped == i), aes(y=z_posture_grouped_per_dataset,x=hours_rounded,color=annotation))+
    ...
    theme(
      strip.background = element_rect(fill="white"))+
    facet_wrap(vars(annotation,posture_grouped),nrow=5)+
    scale_fill_manual(values = pal)+
    scale_color_manual(values = pal)
  
  ggsave(file.path(save_path,paste0("posture_groups_relative_occurences_norm_smoothed_posture_grouped_",i,".png")),height=10,width=3.5)
...
ggplot(data_for_plotting, aes(y=perc_per_posture_grouped_per_dataset,x=hours_rounded,color=annotation))+
  geom_ribbon(aes(ymin = perc_per_posture_grouped_per_annotation-sd_per_posture_grouped_per_annotation_per_tp, ymax = perc_per_posture_grouped_per_annotation + sd_per_posture_grouped_per_annotation_per_tp, fill = annotation),alpha=0.5)+
  ...
  ggsave(file.path(save_path,"posture_groups_relative_occurences.png"),height=6,width=10)
...
# in the paper we use a different annotation
# for OP50 condition
# p1 == posture_group 3
# p2 == posture_group 2
# p3 == posture_group 1
...
s <- cluster_centers_reduced_grouped %>%
  ...
  #manually select 3 posture to be highlighted
  mutate(highlight=ifelse(newID2 %in% c(97,89,21),"YES","NO")) %>%
  group_by(posture_grouped,index) %>%
  mutate(posture_grouped_w_number=paste0(posture_grouped, ", ", n(), " postures"))
...
ggplot(s)+
  ...
  # theme_classic()+
  theme_void()+
  facet_wrap(vars(annotation,posture_grouped_w_number))+
  coord_fixed(ratio = 1)

ggsave(file.path(save_path, "postures_grouped_overlay_OP50_p1vsp2vsp3.png"),height=10,width=10)
```
...
cluster_centers_reduced_grouped %>%
  mutate(posture_meta = ifelse(posture_grouped %in% c("group_1", "group_2"),"curved","dauer")) %>%
...
ggsave(file.path(save_path, "postures_grouped_angle_density_p3vsp1p2.png"),height=3,width=6)
