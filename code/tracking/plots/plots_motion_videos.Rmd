...
skeleton_data_filtered <- files_to_process %>%
  ...
  filter(hours_rounded %in% c(5,11))
...
#only tracks longer than 10 sec (if 2fps)
minimal_track_length <- 20
#How many tracks are shown?
numbers_of_displayed_tracks <- 35
...
for (selected_frame in frames){
  data_for_plotting_temp <- data_for_plotting %>%
    filter(frame_centered == selected_frame)
  ggplot(data_for_plotting_temp)+
    scale_x_continuous(limits= c(-800,800))+
    scale_y_continuous(limits= c(-800,800))+
    #path
    geom_path(data=data_for_plotting,aes(x=location_x_centered, y=location_y_centered,group=track),color="grey",alpha=.25)+
    #skeleton
    geom_point(data=filter(data_for_plotting_temp),aes(location_x_centered+X,location_y_centered+Y,color=angle),size=0.25,alpha=1)+
    #head
    geom_point(data=filter(data_for_plotting_temp,is_head=="YES"),aes(location_x_centered+X,location_y_centered+Y),color="orange",size=1.25,shape=17,alpha=1)+
    # geom_label_repel(data=filter(data_for_plotting_temp, is_head== "YES"),aes(x=location_x_centered, y=location_y_centered,label = track,group=track),
                  # box.padding   = 0.35, 
                  # point.padding = 0.5,
                  # segment.color = 'grey50') +
    scale_color_gradient2(low = "deepskyblue", mid = "lavender",
                          high = "deeppink",midpoint=0,limits =c(-1,1),na.value="white")+
    theme_classic()+
    facet_wrap(vars(hours_rounded))+
    theme(legend.position = "none")
