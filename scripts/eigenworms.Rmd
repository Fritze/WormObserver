...
# ggplot(skeleton_data_filtered,aes(x=velocity))+
#   geom_histogram(binwidth=0.001)+
#   scale_x_continuous(limits=c(0,0.1))
# 
# ggplot(skeleton_data_filtered,aes(x=angle_track))+
#   geom_histogram(binwidth=0.001)
```
...
#         mutate(angle = a*sin(b*t-i)+ c.unif) %>%
#         mutate(seconds = t) %>%
#         #the segment number
#         mutate(index = i)
...
# simulate(e,4.19,30,0.0075,1) %>%
#   filter(index ==5) %>%
#   ggplot(., aes(y=angle,x=seconds))+
#     geom_path()
# 
# simulate(l,3.75,30,0.0075,1) %>%
#   filter(index ==5) %>%
#   ggplot(., aes(y=angle,x=seconds))+
#     geom_path()
# 
# mean_phase_shift <- function(data){
#   max_boo <- rollapply(as.zoo(data$angle), width=3, function(x) which.max(x)==2,align="center")
#   
#   index <- index(max_boo)[coredata(max_boo)]
#   t <- data$seconds[index]
#   mean_ps <- mean(diff(t))
#   data$mean_ps <- mean_ps
#   return(data)
# }
...
#generate simulated worm data, final result is stored in "PC_values_sim"
```{r}

# early_m <- early_s <- simulate(e,4.19,30,0.0075,400) %>%
#   mutate(time = "early") %>%
#   #dummy time
#   mutate(hours_rounded = 1)
# 
# late_m <- simulate(l,3.75,30,0.0075,400) %>%
#   mutate(time = "late") %>%
#   #dummy time
...
#prepare real data for analysing PCA PC_values
```{r}
...
PC_values <- pca$x %>%
  ...
  mutate(time=ifelse(hours_rounded <= 3,"early", "late")) %>%
  ...
  mutate(angle_velo=suppressWarnings(as.numeric(mapply(angle2,diff_prev_PC1,diff_prev_PC2,diff_next_PC1,diff_next_PC2)))) %>%
  #normalize to the range (-pi,pi), this will keep directionality, i.e. clockwise will be negative
  mutate(angle_velo=ifelse(angle_velo < -pi,angle_velo+2*pi,angle_velo)) %>%
  mutate(angle_velo=ifelse(angle_velo > pi, angle_velo-2*pi, angle_velo)) %>%
  ungroup()


saveRDS(PC_values, file.path(save_path,paste0(annotation,"_", number_sampled_tracks,"tracks_PC_values.RDS")))
...
#plotting distribution of local velocity (same subsampled 400 tracks per time range)
```{r}
...
#the plot
ggplot(PC_values,aes(x=velocity,fill=time))+
  geom_density(alpha=0.5)+
  theme_bw()+
  scale_fill_manual(values=c("#E69F00", "#56B4E9"))+
  ggsave(file.path(save_path_temp, paste0(annotation,"_velocity_density.png")),width=plot_size,height=plot_size-1)
  


```

#plotting distribution of phase velocity and local velocity vs phase velocity (same subsampled 400 tracks per time range)
```{r}
...
PC_values %>%
  ...
  filter(numbering != 1) %>%
  ggplot(., aes(x=angle_velo,fill=time))+
    geom_density(alpha=0.5)+
    theme_bw()+
    scale_fill_manual(values=c("#E69F00", "#56B4E9"))+
    ggsave(file.path(save_path_temp, paste0(annotation,"_phase_velocity_density.png")),width=plot_size,height=plot_size-1)

#for correlation plots only look at phase velocity > 0 (forward moving worms)
PC_values %>%
  filter(angle_velo > 0) %>%
  ggplot(., aes(x=velocity, y=angle_velo,fill=time))+
    geom_point(alpha=0.01,shape=21)+
    theme_bw()+
    scale_fill_manual(values=c("#E69F00", "#56B4E9"))+
    facet_wrap(vars(time))+
    ggsave(file.path(save_path_temp, paste0(annotation,"_phase_velocity_correlation_colored.png")),width=plot_size+2,height=plot_size)
...
PC_values %>%
  ...
  ggplot(., aes(x=velocity, y=angle_velo,fill=time))+
    ...
    facet_wrap(vars(time))+
    ggsave(file.path(save_path_temp, paste0(annotation,"_phase_velocity_correlation_viridis.png")),width=plot_size+2,height=plot_size)


```

#plotting distribution of angles for the two timeranges (subsampled to 10k skeletons)
```{r}
#which data to plot
# type "real" or "simulated"
data_type <- "real" #or"simulated"
...
if (data_type == "real"){
#by starting from the PC_values we obtain the same 400 sampled tracks that are used for PCA
thirtyk_skeletons <- data %>%
  ...
  nest() %>%
  group_by(time) %>%
  sample_n(number_sampled_postures) %>%
  unnest(cols=c(data))
