...
plot_posture_examples(data_to_plot, "X","Y") +
  ...
  ggsave(file.path(save_path,paste0("group_",i,"_k_",how_many_clusters,".png")),height=sqrt(number_of_IDs)*5,width=30,limitsize=FALSE)
}
...
load_cc <- function(x){
  readRDS(x) %>%
  mutate(annotation = gsub("^(..+)_angle_data_postures_per_dataset.RDS","\\1",basename(x)))
}
...
ggplot(postures_grouped_statistics, aes(y=perc_norm_per_posture_grouped_per_dataset,x=hours_rounded,color=annotation))+
  ...
  ggsave(file.path(save_path,"posture_groups_relative_occurences_norm.png"),height=7,width=12)

ggplot(postures_grouped_statistics, aes(y=perc_per_posture_grouped_per_dataset,x=hours_rounded,color=annotation))+
  geom_point(alpha=0.5)+
  ...
  strip.background = element_rect(fill="white"))+
  facet_wrap(vars(posture_grouped,annotation),ncol=length(selected_annotation),scales="free")+
  ggsave(file.path(save_path,"posture_groups_relative_occurences.png"),height=7,width=12)
