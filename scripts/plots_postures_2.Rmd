...
ggplot(data_for_plotting, aes(y=perc_per_posture_grouped_per_dataset,x=hours_rounded,color=annotation))+
  ...
  ggsave(file.path(save_path,"posture_groups_relative_occurences.png"),height=4,width=4)
...
cluster_centers_reduced_grouped %>%
  filter(posture_grouped %in% c("group_1","group_2")) %>%
  ggplot(., aes(x=angle,fill=posture_grouped))+
...
s <- cluster_centers_reduced_grouped %>%
  ...
  mutate(X=ifelse(posture_grouped == "group_1",-X-0.25,X)) %>%
