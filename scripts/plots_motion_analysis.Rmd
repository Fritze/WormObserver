...
imported_data <- map_dfr(files_to_process,readRDS) %>%
    ...
    filter(!annotation %in% c("L2","L3","N2 empty after 6h exit","MT13984")) %>%
    na.omit() %>%
...
data_summarised_sub <- data_summarised %>%
  filter(hours_rounded %in% c(hours)) %>%
  filter(behavioral_state == "roaming")

library(jcolors)
ggplot(data_summarised_sub,aes(x=factor(hours_rounded,levels = c(hours)),y=perc))+
...
data_summarised_sub <- data_summarised %>%
  filter(annotation %in% c("Agar", "Agar with NAD","N2 1:10 OP50")) %>%
  filter(hours_rounded %in% c(hours)) %>%
  filter(behavioral_state == "roaming")
...
ggplot(data_summarised_sub,aes(x=factor(hours_rounded,levels = c(hours)),y=perc,fill=annotation))+
      ...
      theme(
        ...
        legend.key.size = unit(2.5, "cm"))+
      ggsave(file.path(save_path,paste0("box_roaming_dwelling_relative_sub_compare_3",".png")),height=5,width=10)
...
ggplot(data_summarised_sub,aes(x=factor(hours_rounded,levels = c(hours)),y=perc,fill=annotation))+
      ...
      theme(
        ...
        axis.line = element_line(colour = 'black', size = 1.5),
        legend.key.size = unit(2.5, "cm"))+
      ggsave(file.path(save_path,paste0("box_roaming_dwelling_relative_sub_compare_2",".png")),height=5,width=10)
```
...
data_binarized %>%
  ...
  ggplot(.,aes(x=p_mean_velocity,fill=annotation))+
    # geom_histogram(lwd=1.2,aes(y=0.05*..density..),binwidth=0.005,color="grey")+
    ...
    facet_wrap(vars(annotation,hours_rounded),ncol=4,scales="free")+
      theme(
        ...
        axis.line = element_line(colour = 'black', size = 1.5),
        legend.key.size = unit(2.5, "cm"))+
      ggsave(file.path(save_path,paste0("Total_average_velocity_two_hours_with_signif_histo_compare",".png")),height=8,width=20)
