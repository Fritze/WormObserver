...
motion_1 <- readRDS(file=file.path(target_folder_base,"200213_Motion_1_1.RDS"))
motion_2 <- readRDS(file=file.path(target_folder_base,"200127_Motion_2.RDS"))
# motion_3 <- readRDS(file=file.path(target_folder_base,"200214_Motion_3_1.RDS"))
...
imported_data_grouped <- imported_data %>%
    filter(
          dataset_ID %in% chosen_exps_1 |
          # dataset_ID %in% chosen_exps_2 |
          dataset_ID %in% chosen_exps_3 ) %>%
          # dataset_ID %in% chosen_exps_4 )  %>%
          # dataset_ID %in% chosen_exps_5) %>%
    ...
    # mutate(annotation = ifelse(dataset_ID %in% chosen_exps_4, "1:5",annotation)) %>%
...
pal <- wes_palette("Darjeeling1",5)[c(1,3)]
...
print(ggplot(perc_turn,aes(x=hours_rounded,y=mean,color=annotation,group=annotation))+
  ...
  geom_ribbon(aes(ymin=mean-se, ymax=mean+se), fill="white",linetype = 0, alpha=0.2)+
...
print(ggplot(perc_turn,aes(x=hours_rounded,y=mean,fill=annotation))+
  ...
  geom_errorbar(position=position_dodge(.45),width=.1, aes(ymin=mean-se, ymax=mean+se),color="white")+
  # geom_ribbon(aes(ymin=lower_ci, ymax=upper_ci), fill="white",linetype = 0, alpha=0.2)+
  # ggtitle(paste0(i," with standard error of the mean"))+
  scale_x_continuous(limits = c(0,9.5),breaks = seq(0, 9, 1))+
  xlab("hours") + ylab("relative occurence of turns")+
  theme_black()+
  theme(legend.direction="vertical")+
...
print(ggplot(perc_straight,aes(x=hours_rounded,y=mean,fill=annotation))+
  geom_bar(stat="identity", position=position_dodge())+
  geom_errorbar(position=position_dodge(.5),width=.1, aes(ymin=mean-se, ymax=mean+se),color="white")+
  # geom_ribbon(aes(ymin=lower_ci, ymax=upper_ci), fill="white",linetype = 0, alpha=0.2)+
  # ggtitle(paste0(i," with standard error of the mean"))+
  scale_x_continuous(limits = c(0,9),breaks = seq(0, 9, 1))+
  xlab("hours") + ylab("relative occurence of straight movement")+
...
ggplot(boxplot,aes(x=as.character(hours_rounded),y=p_length,fill=annotation))+
      geom_boxplot()+
      # geom_violin(draw_quantiles=1)+
      #ggtitle(paste0(i," with standard error of the mean"))+
      # scale_x_continuous(limits = c(0,9),breaks = seq(0, 9, 1))+
...
ggplot(heatmap,aes(x = p_mean_angle, y = p_mean_velocity))+
  ...
  theme_black()+
  coord_fixed(ratio = 1)+
  facet_wrap(annotation+hours_rounded ~ ., nrow=length(unique(heatmap$annotation)), ncol= 18)+
...
imported_data_grouped %>%
  filter(hours_rounded <= 9.0)  %>%mutate(ratio = p_mean_velocity/p_mean_angle) %>%
