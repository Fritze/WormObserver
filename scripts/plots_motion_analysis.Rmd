...
save_path <- file.path(save_path, average)
dir.create(save_path)
...
ggplot(data,aes(x = p_mean_angle, y = p_mean_velocity))+
  ...
  ggsave(file.path(save_path,paste0("scatter_roaming_dwelling_heatmap_show_all_points_and_density",".png")),bg="black",height=25,width=35)
...
ggplot(data,aes(x = p_mean_angle, y = p_mean_velocity))+
  ...
  theme(strip.background = element_rect(colour = "white", fill = "white"),
      ...
      axis.title.y = element_text(size=40))+
  facet_wrap(vars(annotation,hours_rounded,number_of_tracks_in_this_hour),ncol=length(unique(data$hours_rounded)),scales="free_x")+
  ggsave(file.path(save_path,paste0("scatter_roaming_dwelling_heatmap_show_density",".png")),bg="black",height=25,width=35)
...
data_summarised_sub <- data_summarised %>%
  filter(hours_rounded %in% c(first_hour,other_hour)) %>%
  filter(behavioral_state == "roaming")

library(jcolors)
ggplot(data_summarised_sub,aes(x=factor(hours_rounded,levels = c(first_hour,other_hour)),y=perc))+
      geom_boxplot(aes(fill=annotation),color="black",width=0.2,outlier.alpha = 0.3,outlier.size = 1,lwd=0.5)+
...
data_summarised_sub <- data_summarised %>%
  filter(annotation %in% c("Agar", "N2 1:10 OP50")) %>%
  filter(hours_rounded %in% c(first_hour,other_hour)) %>%
  filter(behavioral_state == "roaming")

library("wesanderson")
ggplot(data_summarised_sub,aes(x=factor(hours_rounded,levels = c(first_hour,other_hour)),y=perc,fill=annotation))+
      ...
      theme(
        ...
        legend.key.size = unit(2.5, "cm"))+
      ggsave(file.path(save_path,paste0("box_roaming_dwelling_relative_sub_compare_OP50andAgar",".png")),height=5,width=7)

data_summarised_sub <- data_summarised %>%
  filter(annotation %in% c("Agar", "Agar with NAD")) %>%
  filter(hours_rounded %in% c(first_hour,other_hour)) %>%
  filter(behavioral_state == "roaming")

data_summarised_sub$annotation <- factor(data_summarised_sub$annotation, levels=rev(unique(data_summarised_sub$annotation)))

library("wesanderson")
ggplot(data_summarised_sub,aes(x=factor(hours_rounded,levels = c(first_hour,other_hour)),y=perc,fill=annotation))+
      geom_boxplot(aes(fill=annotation),lwd=1.2)+
      geom_point(aes(fill=annotation),position = position_jitterdodge(),shape=21,size=2.5)+
      xlab("hours") + ylab("Relative occurence of roaming")+
      scale_y_continuous(limits=c(0,1))+
      scale_fill_manual(values=wes_palette(n=3, name="Moonrise3"))+
      theme_classic()+
      theme(
        ...
        axis.line = element_line(colour = 'black', size = 1.5),
        legend.key.size = unit(2.5, "cm"))+
      ggsave(file.path(save_path,paste0("box_roaming_dwelling_relative_sub_compare_AgarandAgarNAD",".png")),height=5,width=7)
```
...
data_binarized_only <- data_binarized %>%
  filter(behavioral_state == "roaming") %>%
...
# 
# data_binarized %>%
#   filter(annotation %in% c("N2 1:10 OP50")) %>%
#   filter(hours_rounded %in% c(first_hour,other_hour)) %>%
#   ggplot(.,aes(x=factor(hours_rounded,levels = c(first_hour,other_hour)),y=p_mean_velocity,color=annotation))+
#       geom_boxplot(lwd=1.2,outlier.shape = 21)+
#       geom_violin(lwd=0,fill="lightgrey", alpha=0.8)+    
#       xlab("hours") + ylab("velocity (mm/s)")+
#       scale_color_manual(values=wes_palette(n=1, name="Moonrise3"))+
#       theme_classic()+
#       theme(
#         legend.direction="horizontal",
#         legend.position = "top",
...
#         axis.line = element_line(colour = 'black', size = 1.5),
#         legend.key.size = unit(2.5, "cm"))+
#       ggsave(file.path(save_path,paste0("total_average_velocity_two_hours_with_signif_compare",".png")),height=5,width=7)

data_binarized %>%
  filter(annotation %in% c("N2 1:10 OP50")) %>%
  filter(hours_rounded %in% c(first_hour,other_hour)) %>%
  ggplot(.,aes(x=p_mean_velocity))+
    geom_histogram(lwd=1.2,aes(y=0.005*..density..),binwidth=0.005,color="grey")+
    xlab("velocity (mm/s)")+
    theme_classic()+
    facet_wrap(vars(hours_rounded))+  
      theme(
        ...
        axis.line = element_line(colour = 'black', size = 1.5),
        legend.key.size = unit(2.5, "cm"))+
      ggsave(file.path(save_path,paste0("total_average_velocity_two_hours_with_signif_histo_compare",".png")),height=5,width=7)


##### dwelling
data_binarized_only <- data_binarized %>%
  filter(behavioral_state == "dwelling") %>%
  select(hours_rounded, annotation, p_mean_velocity)
...
data_binarized_only %>%
  ...
  theme(legend.direction="horizontal",
        ...
        legend.key.size = unit(2.5, "cm"))+
  facet_wrap(vars(annotation),scales="free_x")+
  ggtitle("Velocity average, only for dwelling fraction")+
  ggsave(file.path(save_path,paste0("violin_dwelling_average_velocity_two_hours_with_signif",".png")),height=10,width=10,dpi=300)
