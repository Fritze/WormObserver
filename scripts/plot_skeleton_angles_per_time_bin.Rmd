...
# numbers_of_tracks_per_time <- skeleton_data_filtered %>%
#    filter(tp %in% 1:40) %>%
#    mutate(time = ifelse(tp < 8,"<1h",NA)) %>%
#    mutate(time = ifelse(tp >= 8 & tp < 16,"<2h",time)) %>%
#    mutate(time = ifelse(tp >= 16 & tp < 24,"<3h",time)) %>%
...
```{r, include=FALSE}
x<-bin_data_sampled_head$angle
filter(bin_data_sampled_head,angle > 1 )


scaleggplot(bin_data_sampled_head, aes(x=angle,y=index),group=time )+
   # geom_bin2d()+
   # geom_hex()+
   # stat_density_2d(aes(fill = ..density..), geom = "polygon")+
   stat_density_2d(aes(fill = ..ndensity..), geom = "raster", contour = FALSE) +
