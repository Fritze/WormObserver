...
target_folder<-"/media/fpreuss/raid5/timelapses/analysis/200612/data/posture/clustered_1"
save_path <- "/media/fpreuss/raid5/timelapses/analysis/200612/plots/posture/heatmaps"
...
save_path_comparison <- "/media/fpreuss/raid5/timelapses/analysis/200612/plots/posture/posture_comparison_plots"
...
for (i in unique(skeleton_data_clustered_subset$frame)){
  ...
  ((plot_raw + plot_spacer()) |
    ...
    # plot_layout(widths=c(1,3,3), heights = c(1,3, 3))
     ggsave(file.path(save_path_comparison,paste0("posture_frame",selected_frame,".png")),height=15,width=10)
...
for (a in annotations){
 ...
 plot_posture_heatmap(heatmap_data_to_plot_per_dataset,ordering) +
  ...
  heatmap /
    ...
    ggsave(file.path(save_path,paste0("200511_heatmap_",paste0(a,collapse="+"),"_",n,".png")),width=49,height=30)
...
```{r, include=FALSE}
library(ggsignif)
all_IDs = unique(skeleton_data_clustered)
for ()
oneID <- angle_data_postures_per_dataset %>%
  filter(newID == 1) %>%
  mutate(time_bin = ifelse(hours_rounded <= 3, "< 3","> 3")) %>%
  # filter(annotation == "Agar") %>%
  select(annotation,perc_per_dataset,time_bin) %>%
  group_by(annotation) %>%
  do(w = wilcox.test(perc_per_dataset~time_bin, data=., paired=FALSE)) %>%
  summarise(annotation, Wilcox = w$p.value)

wilcox.test(x, y, alternative = "two.sided")


data_to_plot <- angle_data_postures_per_dataset %>%
  filter(newID %in% c(79,29,33)) %>%
  filter(hours_rounded <= 6) %>%
  mutate(time_bin = ifelse(hours_rounded <= 3, "< 3","> 3"))

ggplot(data_to_plot, aes(x=time_bin, y=perc_per_dataset,color=annotation))+
  geom_boxplot()+
  geom_point(alpha=0.5) +
  facet_wrap(vars(newID,annotation),ncol = 2,scales="free")+
  theme_classic()+
   ggsave(file.path(save_path,paste0("comparison_test",".png")),width=10,height=5)
...
save_path <- "/home/fpreuss/raid/timelapses/analysis/200318/plots/posture/syntax/"
...
posture_trigrams_w_postures <- posture_trigrams_stats %>%
  ...
  filter(newID_sub == 1) %>%
  select(-c(clusterID, angle_mirror, mirroredID,difference, newID_sub)) %>%
