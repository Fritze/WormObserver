...
#How many worms/tracks per timepoint?
```{r, include=FALSE}
angle_data_postures %>%
  filter(clusterID == 1) %>%
  mutate(mean_tracks_per_tp=mean(tracks_per_tp))

```

#How many postures per timepoint?
```{r, include=FALSE}
angle_data_postures %>%
  filter(clusterID == 1) %>%
  mutate(mean_postures_per_tp=mean(postures_per_tp))

```
#How many postures of a certain type per timepoint?
```{r, include=FALSE}
angle_data_postures %>%
  filter(clusterID == 1) %>%
  mutate(mean_cluster_members_per_tp=mean(number_of_cluster_members_per_tp))
...
p <- ggplot(clustered_skeleton_list, aes(x=X,y=Y,colour=angle))+
