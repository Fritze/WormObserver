...
```{r, include=FALSE}
#
cluster <- kmeans(angle_data_cluster,centers=150,iter.max = 1000,algorithm = "Lloyd")
...
saveRDS(angle_data_clustered,file.path(save_path,paste0("nofood_3_datasets_clustered.rds")))
...
p <- ggplot(freq_postures, aes(x=rank, y=perc))+
  ...
  theme_black()
p
ggsave(file.path(save_path,paste0("nofood_150_postures_ranked",".png")),width=20,height=11)
```
...
angle_data_postures <- angle_data_clustered %>%
  group_by(clusterID,hours_rounded) %>%
  ...
  select(clusterID,hours_rounded, postures_per_tp,tracks_per_tp, perc,perc_norm,ID,number_of_cluster_members_per_tp) %>%
  group_by(clusterID,hours_rounded) %>%
...
cluster_clusters <- angle_data_postures  %>%
   dcast(.,clusterID ~ hours_rounded,value.var = "perc_norm")
...
angle_data_postures_filtered <- filter(angle_data_postures,clusterID %in% freq_postures$clusterID)

ggplot(angle_data_postures_filtered,aes(x=factor(clusterID,levels=freq_postures$clusterID),y=hours_rounded))+
  geom_tile(aes(fill=perc_norm))+
  theme_classic()+
  scale_fill_distiller(palette = "PiYG",limits=c(-1,1) * max(abs(angle_data_postures$perc_norm)))+
  labs(x="posture IDs",y = "hours") +
  theme_black()+
  # facet_wrap(~dataset_ID,nrow=3)+
...
ggsave(file.path(save_path,paste0("nofood_skeletons_freq_heatmap_percent_all_postures_per_dataset_20mostabundant",".png")),width=20,height=13)
```
