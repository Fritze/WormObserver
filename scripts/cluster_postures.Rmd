...
saveRDS(skeleton_data_filtered, file=file.path(target_folder_base,"200214_Skeletons_filtered_clustered.RDS"))
...
angle_data_postures <- angle_data_clustered %>%
  ...
  group_by(clusterID,hours,annotation) %>%
  mutate(number_of_cluster_members_per_tp=n_distinct(ID)) %>%
  group_by(hours,annotation) %>%
...
ggplot(angle_data_postures_filtered,aes(x=factor(clusterID,levels=ordering),y=hours_rounded))+
