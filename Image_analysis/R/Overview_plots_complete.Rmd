...
target_folder_general <- c("2019-03-06_17-56-39")
                           # "2019-03-07_17-28-51",
                           # "2019-03-11_17-54-09",
                           # "2019-03-18_18-01-40",
                           # "2019-03-19_17-40-32",
...
ds <- read.csv("/media/ella/raid5/example.csv")

ds_test <- ds %>%
  filter(TrackID == "track_10")


i <- 2
dim_X <- ds_test$bitmask_dim_X[i]
dim_Y <- ds_test$bitmask_dim_Y[i]
matrix <- as.integer(gsub(".*(\\d+)\\..+","\\1",unlist(strsplit(as.character(ds_test$bitmask[i]), ",")))) %>%
  matrix(.,nrow=dim_Y, byrow=TRUE) %>%
  melt(.) %>%
  rename(Y = 1, X = 2,value = 3)%>%
  filter(value == 1) %>%
  mutate(dx = X - lag(X)) %>%
  mutate(dy = Y - lag(Y)) %>%
  mutate(distance = sqrt(dy^2 + dx^2), angle = atan2(dy, dx)) %>%
  mutate(angle = angle*180/pi)

pal <- wes_palette(name = "Zissou1", type = "continuous")
ggplot(matrix, aes(x=X,y=Y,fill=angle))+
  coord_equal() + 
  geom_raster()+
