...
skeleton("2019-03-26_19-27-16",TrackIDs,tps,0:450)
...
# skeleton_data_all$TrackCheck_cleaned <- replace_f(skeleton_data_all$TrackCheck, 24*10)
...
for (f in unique(skeleton_data_all_cleaned$frame)){
    ...
    p <- ggplot(data_plot, aes(x=X,y=Y,colour=angle_1))+
      ...
      geom_point(data=data_plot[which(data_plot$is_head == "YES"), ], aes(x=X, y=Y), shape=19, colour = "gold", size=1.5, alpha=0.8) +
      ...
      scale_color_gradient2(low = "cornflowerblue", mid = "white",
                         high = "red",midpoint=0,limits=c(-0.7,0.7)) +
      coord_equal() +
      theme(legend.position = "none",
...
skeleton_m <- skeleton_data_all %>%
  # filter(TrackID == "track_12") %>%
  group_by(frame) %>% 
...
ggplot(skeleton_m, aes(x=X,y=Y,fill=angle_1))+
  geom_raster(na.rm=TRUE) +
  scale_x_continuous(limits=c(0,200)) +
  facet_wrap(~grouping) +
  scale_fill_gradient2(low = "cornflowerblue",mid = "white",high = "brown2",midpoint=0,limits=c(-0.7,0.7)) +
  theme_void()+
  theme(strip.text.x = element_text(size = 3))



 ggsave(paste0(file.path(target_folder_analysis,"ggplots"),"/","test_waves",".png"),height = 60,width= 60,units = "cm",bg="black",dpi=150)
...
expca <- skeleton_data_all %>%
  # filter(TrackID == "track_12") %>%
  filter(tp == 35) %>%
  select(angle_1,index, frame,TrackCheck)

expca$TrackCheck_cleaned <- replace_f(expca$TrackCheck, 24*120)
...
expca_t <- dcast(expca, index~time,value.var = "angle_1")


expca_t <- reshape(expca, timevar = c("time"), idvar = "index", direction = "wide", v.names = c("angle_1"))


??reshape2

?reshape
expca_t <- dcast(expca, index~frame,value.var = "angle_1")
rownames(expca_t) <- expca_t$index
expca_t <- select(expca_t, -index)
pca <- prcomp(expca_t,scale=TRUE)
plot(pca)
pca$rotation
eigenworms <- data.frame(pca$x[,1:6])

ev <- data.frame(pca$sdev^2)
colnames(ev) <- "eigenvalue"
ev$cumsum_eigenvalue <- cumsum(ev$eigenvalue)

ev <- ev %>%
  mutate(var_contr = cumsum_eigenvalue/sum(eigenvalue)) %>%
  mutate(index = 1:n()) %>%
  filter(index %in% c(1:5))


ggplot(ev,aes(index,var_contr)) +
  geom_line() +
