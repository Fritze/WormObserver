...
data_skeleton <- imported_data %>%
  ...
  filter(TrackID %in% trackID_names) %>%
...
# data_skeleton_head <- data_skeleton %>%
#   group_by(dataset_ID,tp,TrackID,frame) %>%
#   nest() %>%
#   mutate(mh = future_map(data,skeleton_head)) %>%
#   select(dataset_ID,tp,TrackID,frame,mh) %>%
#   unnest() %>%
#   select(-ends_with("1"))
...
save(data_skeleton, file = paste0("2019-03-26_19-27-16","_","23tps","_","0-900","_no_head",".RData"))
